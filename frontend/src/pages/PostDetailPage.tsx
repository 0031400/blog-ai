    ...
    <>
      <section className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-900">阅读信息</h3>
          <span className="text-[11px] uppercase tracking-[0.14em] text-slate-400">Meta</span>
        </div>
        <div className="mt-3 space-y-2">
          {[
            [post ? post.category : '未分类', '文章分类'],
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-900">状态</h3>
          <span className="text-[11px] uppercase tracking-[0.14em] text-slate-400">Sync</span>
        </div>
        <div className="mt-3 space-y-2">
          <div className="rounded-xl bg-slate-50 px-3 py-3">
            <div className="text-sm font-medium text-slate-900">{detailLoading ? '同步中' : '已加载'}</div>
            <p className="mt-1 text-sm leading-6 text-slate-500">{detailLoading ? '正在请求最新文章内容。' : '当前显示的是可阅读正文。'}</p>
          </div>
          <div className="rounded-xl bg-slate-50 px-3 py-3">
            <div className="text-sm font-medium text-slate-900">Hash Route</div>
            <p className="mt-1 text-sm leading-6 text-slate-500">仍然沿用 `/#/posts/:slug` 路由。</p>
          </div>
        </div>
      </section>
    </>
      ...
      main={
        <>
          <section className="mb-3 flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200/80 bg-white/85 p-2 shadow-sm">
            <a href={createHomeHref()} className="rounded-xl bg-slate-900 px-3 py-2 text-sm text-white">
              返回首页
            </a>
            <span className="rounded-xl px-3 py-2 text-sm text-slate-500">{detailLoading ? '正在同步' : '文章详情'}</span>
          </section>

          {detailError ? <div className="mb-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">{detailError}</div> : null}
          ...
          {post ? (
            <article className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white/90 shadow-sm">
              ...
              <div className="p-5 md:p-6">
                ...
                <h1 className="mt-3 text-[30px] font-semibold tracking-[-0.05em] text-slate-900 md:text-[34px]">{post.title}</h1>
                ...
                <div className="mt-4 flex items-center gap-3 rounded-2xl bg-slate-50 px-3 py-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-500 text-sm font-semibold text-white">B</div>
                  <div>
                    <strong className="text-sm text-slate-900">blog-ai</strong>
